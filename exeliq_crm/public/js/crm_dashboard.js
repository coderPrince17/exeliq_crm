...
(function () {
	...
	function registerPage() {
		...
		// Strategy: try immediately, then retry at 200ms, 500ms, 1000ms.
		function _try_init() {
			if (frappe.crm_dashboard) return; // already done

			// Check if we're on the crm-dashboard route
			...
			container.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;' +
				'background:var(--bg-color,#fff);overflow-y:auto;z-index:1';
			document.body.appendChild(container);
			loadChartJS(function () { initDashboard(container); });
		}

		// Try at multiple intervals to handle different load timings
		_try_init();
		setTimeout(_try_init, 200);
		setTimeout(_try_init, 500);
		setTimeout(_try_init, 1000);
		setTimeout(_try_init, 2000); // last resort for very slow connections
	}
