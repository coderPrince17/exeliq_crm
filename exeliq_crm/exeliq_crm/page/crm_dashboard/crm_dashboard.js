...
(function () {
	...
	function registerPage() {
		...
		// Race condition fix: JS loads after router already navigated here.
		// Try once with URL check — if on crm-dashboard, create container and init.
		setTimeout(function () {
			...
			c.setAttribute('data-page-route', 'crm-dashboard');
			c.style.cssText = 'position:fixed;top:56px;left:0;right:0;bottom:0;background:var(--bg-color,#fff);overflow-y:auto;z-index:10';
			document.body.appendChild(c);
			loadChartJS(function () { initDashboard(c); });
		}, 500);
	}
	...
	CRMDashboard.prototype.setup_html = function () {
		...
		// Wire toggles
		var hm_count = document.getElementById('hm-count');
		var hm_value = document.getElementById('hm-value');
		var hm_both  = document.getElementById('hm-both');
		if (hm_count) hm_count.onclick = function(){ self.set_metric('count',this,'opp'); };
		if (hm_value) hm_value.onclick = function(){ self.set_metric('value',this,'opp'); };
		if (hm_both)  hm_both.onclick  = function(){ self.set_metric('both', this,'opp'); };
	...
	CRMDashboard.prototype.mount_filters = function () {
		...
		if (!hc) return;
		// Remove stale filter bar if exists
		var old = hc.querySelector('.page-form.row');
		if (old && old !== pf[0]) old.parentElement.removeChild(old);
		pf[0].id = 'crm-pf';
		hc.appendChild(pf[0]);
	...
	CRMDashboard.prototype.render_opp_heatmap = function () {
		...
		el.innerHTML=this._hm_table(users, self.stages,
			function(r){return r===null?'User':self.sp_name(r);},
			function(u,s,mode){
				...
				if(mode==='v') return metric==='value'?v:c;
				if(mode==='l') return metric==='value'?fmt(v):String(c);
				if(mode==='s') return (metric==='both'&&c>0)?fmt(v):'';
			},
			'crm-opp-hm-legend'
		);
	};
	...
	CRMDashboard.prototype.render_lead_heatmap = function () {
		...
		el.innerHTML=this._hm_table(data, statuses,
