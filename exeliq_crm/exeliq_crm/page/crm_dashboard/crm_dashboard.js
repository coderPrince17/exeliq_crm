// Exeliq CRM Dashboard v17
// Uses server-side API: exeliq_crm.exeliq_crm.api.dashboard.get_dashboard_data
// Filters load via app_include_js — no dependency on frappe.client.get_list for meta
...
(function () {
	...
	function CRMDashboard(page) {
		...
		this.opps             = [];
		this.leads            = [];
		this.stages           = [];        // [{name, label}]
		this.stage_color_map  = {};
		this.territories      = [];        // all Territory records
		this.salespersons     = [];        // [{user, full_name}]
		this.kpis             = {};
		this.funnel           = [];        // converted to array [{stage, count, value}]
	...
	CRMDashboard.prototype.setup_filters = function () {
		...
		// Options populated after first API call returns salespersons list
		if (this.is_mgr) {
	...
	CRMDashboard.prototype.get_f = function () {
		var sp = null;
		if (this.is_mgr && this.f_sp) {
			var v = this.f_sp.get_value();
			// f_sp stores email — but display may be full_name
			// We store email in _sp_email_map
			if (v && v !== 'All') {
				sp = this._sp_email_map ? (this._sp_email_map[v] || v) : v;
			}
		}
		return {
	...
	CRMDashboard.prototype.load_data = function () {
		...
		frappe.call({
			...
			callback: function (r) {
				...
				if (!r.message) {
					document.getElementById('crm-metrics').innerHTML =
						'<div style="color:var(--red);padding:10px">No data returned. Check console.</div>';
					return;
				}
				...
				// Funnel: API returns dict {stage: {count, value}}
				// Convert to sorted array using stages order
				var funnel_dict = d.funnel || {};
				self.funnel = self.stages.map(function (s) {
					var row = funnel_dict[s] || { count: 0, value: 0 };
					return { stage: s, count: row.count, value: row.value };
				});

				// Salespersons: [{user, full_name}]
				self.salespersons = d.salespersons || [];

				// Update salesperson dropdown with full names
				if (self.f_sp && self.salespersons.length) {
					self._sp_email_map = {}; // display → email
					var opts = ['All'];
					self.salespersons.forEach(function (sp) {
						var display = sp.full_name || user_label(sp.user);
						opts.push(display);
						self._sp_email_map[display] = sp.user;
					});
					self.f_sp.df.options = opts.join('\n');
					self.f_sp.refresh();
				}
			...
			error: function (err) {
				...
				console.error('CRM Dashboard API error:', err);
			}
		});
	};
	...
	// ── Metrics (from pre-calculated kpis) ───────────────────────────
	...
	// ── Pipeline strip (from funnel array) ────────────────────────────
	...
	CRMDashboard.prototype.render_pipeline = function () {
		...
		}).join('') || '<div class="crm-empty">No stage data</div>';
	};

	// ── Stage bar chart (pipeline only, excl Won/Lost) ────────────────
	...
	CRMDashboard.prototype.render_stage_chart = function () {
		...
		var pipeline_funnel = this.funnel.filter(function (r) {
			return r.stage !== WON_STAGE && r.stage !== LOST_STAGE;
		});
		if (!pipeline_funnel.length) return;

		this.charts['stage'] = new Chart(ctx, {
			type: 'bar',
			data: {
				labels: pipeline_funnel.map(function (r) {
					return r.stage.length > 12 ? r.stage.slice(0, 12) + '…' : r.stage;
				}),
				datasets: [{
					label: 'Deals',
					data: pipeline_funnel.map(function (r) { return r.count; }),
					backgroundColor: pipeline_funnel.map(function (r) { return self.stage_color(r.stage); }),
					borderRadius: 3
				}]
			},
			options: {
	...
	// ── Territory chart — ALL territories from doctype (half width, vertical bars) ──

	CRMDashboard.prototype.render_territory = function () {
		var ctx = document.getElementById('crm-territory-chart');
		if (!ctx || !window.Chart) return;

		// Use territory_stats from API which includes ALL territories
		// Filter out groups, show only leaf territories or those with data
		var stats = this.territory_stats.filter(function (t) {
			return t.opportunities > 0 || t.leads > 0;
		});

		// If nothing has data, show all territories with zeros
		if (!stats.length) {
			stats = this.territory_stats.slice(0, 15);
		}

		// Sort by total activity desc, take top 15
		stats = stats.slice().sort(function (a, b) {
			return (b.opportunities + b.leads) - (a.opportunities + a.leads);
		}).slice(0, 15);

		if (!stats.length) {
			ctx.parentElement.innerHTML = '<div class="crm-empty">No territory data</div>';
			return;
		}
		...
		this.charts['territory'] = new Chart(ctx, {
			...
			data: {
				...
				datasets: [
					{
						label: 'Opportunities',
						data: stats.map(function (t) { return t.opportunities; }),
						backgroundColor: '#378ADD', borderRadius: 3
					},
					{
						label: 'Leads',
						data: stats.map(function (t) { return t.leads; }),
	...
	// ── Sales Funnel (horizontal bar progress chart) ──────────────────
	...
	CRMDashboard.prototype.render_funnel = function () {
		...
		var pipeline_funnel = this.funnel.filter(function (r) {
			return r.stage !== LOST_STAGE;
		});

		if (!pipeline_funnel.length) {
			container.innerHTML = '<div class="crm-empty">No opportunities found</div>';
			return;
		}

		var max_count = pipeline_funnel[0] ? pipeline_funnel[0].count : 1;

		container.innerHTML = pipeline_funnel.map(function (row, i) {
			var pct = max_count > 0 ? Math.round((row.count / max_count) * 100) : 0;
	...
	CRMDashboard.prototype.hm_users = function () {
		...
		// Use salespersons from API, fall back to users from opps
		var seen = {}, result = [];
		this.salespersons.forEach(function (sp) {
			if (sp.user && !seen[sp.user]) { seen[sp.user] = 1; result.push(sp.user); }
		});
	...
	CRMDashboard.prototype.sp_display = function (u) {
		if (!u) return '-';
	...
	CRMDashboard.prototype.render_heatmap = function () {
		...
		if (!users.length || !self.stages.length) {
			document.getElementById('crm-heatmap').innerHTML = '<div class="crm-empty">No data</div>';
			return;
		}
		...
		users.forEach(function (u) {
			var u_c = 0, u_v = 0;
			html += '<tr><th class="row-h">' + self.sp_display(u) + '</th>';
			self.stages.forEach(function (s) {
				var items = self.opps.filter(function (o) { return o.opportunity_owner === u && o.sales_stage === s; });
				var c = items.length;
		...
		self.stages.forEach(function (s) {
			...
			var sc = sr.length ? sr[0].count : 0;
			var sv = sr.length ? sr[0].value : 0;
	...
	CRMDashboard.prototype.render_sp_perf = function () {
		...
		if (!this.is_mgr) {
			// Find current user in salesperson_stats
			var me = this.salesperson_stats.filter(function (s) { return s.owner === self.me; })[0];
			if (!me) {
				container.innerHTML = '<div class="crm-empty">No data for selected period</div>';
				return;
			}
		...
		container.innerHTML = this.salesperson_stats.map(function (sp) {
			...
			var name   = self.sp_display(sp.owner);
