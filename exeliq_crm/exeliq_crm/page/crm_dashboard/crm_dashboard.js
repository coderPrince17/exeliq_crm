...
(function () {
	...
	function registerPage() {
		...
		// Race condition fix:
		// app_include_js loads AFTER Frappe router already tried to render
		// crm-dashboard. on_page_load was undefined at that point so nothing
		// rendered. Frappe won't retry. We must trigger it ourselves.
		//
		// make_app_page needs a real DOM element as parent.
		// Frappe normally creates .page-container divs inside .page-body,
		// but for non-admin users the page-body doesn't exist yet.
		// We create the container ourselves and append to document body.
		setTimeout(function () {
			...
			// Check if Frappe already created a container
			...
			// Create container ourselves — Frappe's make_app_page will
			// populate it with the page structure
			var container = document.createElement('div');
			container.className = 'page-container';
			container.setAttribute('data-page-route', 'crm-dashboard');
			container.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:var(--bg-color,#fff);overflow-y:auto;z-index:1';

			// Find the best place to attach — desk-body, main, or body
			var host = document.querySelector('.desk-body')
...
.crm-mgr-only{/* shown only to managers */}
  ...
  <div class="crm-card crm-mgr-only">
    ...
    <div class="crm-toggle-row">
      <button class="crm-toggle active" id="lhm-count">Lead count</button>
    </div>
  ...
  <!-- Monthly Stage Heatmap: months vs stages (Sales Users only) -->
  <div class="crm-card">
    <div class="crm-section-title">My pipeline — months vs stages</div>
    <div class="crm-toggle-row">
    ...
    <div class="crm-legend"><span>Low</span><div class="crm-legend-strip" id="crm-mshm-legend"></div><span>High</span></div>
		...
		var quarterly_cards = mgr ? `
  <!-- Quarterly Revenue (managers only) -->
  <div class="crm-card crm-mgr-only">
...
<div class="crm-dash">
  ...
  <div class="crm-card">
	...
	CRMDashboard.prototype.render_all = function () {
		this.render_metrics();
		if(this.is_mgr) this.render_quarterly();
			...
			options:{responsive:true,maintainAspectRatio:false,
				plugins:{legend:{position:'right',labels:{font:{size:11},boxWidth:12,padding:10,usePointStyle:true}},
					tooltip:{callbacks:{label:function(ctx){
						return ' '+self.sp_name(data[ctx.dataIndex].owner)+': '+fmt(ctx.raw);
					}}}}}
		});
	};
	...
	CRMDashboard.prototype.render_monthly_stage_heatmap = function () {
		...
		el.innerHTML=this._hm_table(
			...
			function(r,month,mode){
				if(r==='header') return 0;
				var cell=(matrix[r.stage]&&matrix[r.stage][month])||{count:0,value:0};
