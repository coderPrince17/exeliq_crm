// Exeliq CRM Dashboard v19
// Fix: Inject filters into page.page_form and force it visible
...
(function () {
	...
	// ── Step 1: Setup page content (main area) ────────────────────────
...
<div class="crm-dash">
  ...
  <div class="crm-card">
	...
	// ── Step 2: Setup filters in page_form ───────────────────────────
	// page.page_form is the correct Frappe v15 filter container
	// It exists but has height:0 — we populate it and make it visible

	CRMDashboard.prototype.setup_filters = function () {
		var self = this;
		var pf = this.page.page_form; // jQuery object wrapping .page-form.row

		if (!pf || !pf.length) {
			console.warn('CRM: page_form not found, filters unavailable');
			return;
		}

		// page_form is detached from the DOM in Frappe v15 with single_column:true
		// We must manually insert it into the page-head container
		var head_container = document.querySelector('.page-head .container');
		if (head_container) {
			head_container.appendChild(pf[0]);
		}
		pf[0].style.cssText = 'display:flex;flex-wrap:wrap;gap:8px;padding:8px 0;align-items:flex-end;width:100%';

		// Build filter HTML inside page_form
		var def = this.period_dates('This Month');
		var sp_html = this.is_mgr
			? '<div class="crm-pf-group" id="crm-sp-group">' +
			  '<label class="crm-pf-label">Salesperson</label>' +
			  '<select id="crm-f-sp" class="crm-pf-select"><option value="">All</option></select></div>'
			: '';

		pf[0].innerHTML = `
<style>
.crm-pf-group{display:inline-flex;flex-direction:column;gap:3px;margin-right:12px;vertical-align:bottom}
.crm-pf-label{font-size:10px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.04em;font-weight:500}
.crm-pf-select,.crm-pf-date{font-size:12px;padding:4px 8px;border:1px solid var(--border-color);border-radius:4px;
  background:var(--fg-color);color:var(--text-color);height:28px;outline:none;cursor:pointer;min-width:110px}
.crm-pf-select:focus,.crm-pf-date:focus{border-color:var(--blue)}
.crm-pf-btn{display:inline-block;padding:5px 14px;font-size:12px;border:1px solid var(--blue);
  border-radius:4px;background:var(--blue);color:#fff;cursor:pointer;height:28px;vertical-align:bottom;line-height:1.4}
.crm-pf-btn:hover{opacity:.9}
</style>
<div style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:4px;padding:4px 0">
  <div class="crm-pf-group">
    <label class="crm-pf-label">Range</label>
    <select id="crm-f-period" class="crm-pf-select">
  ...
  <div class="crm-pf-group">
    <label class="crm-pf-label">From</label>
    <input type="date" id="crm-f-from" class="crm-pf-date" value="${def[0]}" />
  </div>
  <div class="crm-pf-group">
    <label class="crm-pf-label">To</label>
    <input type="date" id="crm-f-to" class="crm-pf-date" value="${def[1]}" />
  </div>
  ${sp_html}
  <button class="crm-pf-btn" id="crm-refresh-btn">↻ Refresh</button>
</div>`;

		// Wire up events
		document.getElementById('crm-f-period').onchange = function () {
			var p = this.value;
			if (p === 'Custom') return;
			var d = self.period_dates(p);
			document.getElementById('crm-f-from').value = d[0];
			document.getElementById('crm-f-to').value   = d[1];
			self.load_data();
		};

		document.getElementById('crm-f-from').onchange = function () {
			document.getElementById('crm-f-period').value = 'Custom';
			self.load_data();
		};

		document.getElementById('crm-f-to').onchange = function () {
			document.getElementById('crm-f-period').value = 'Custom';
			self.load_data();
		};

		if (this.is_mgr) {
			document.getElementById('crm-f-sp').onchange = function () { self.load_data(); };
		}

		document.getElementById('crm-refresh-btn').onclick = function () { self.load_data(); };
	};
	...
	CRMDashboard.prototype.get_f = function () {
		var from_el   = document.getElementById('crm-f-from');
		var to_el     = document.getElementById('crm-f-to');
		var sp_el     = document.getElementById('crm-f-sp');
		return {
			from: (from_el && from_el.value) || frappe.datetime.month_start(),
			to:   (to_el   && to_el.value)   || frappe.datetime.now_date(),
			sp:   (this.is_mgr && sp_el && sp_el.value) ? sp_el.value : null
		};
	};
	...
	CRMDashboard.prototype.load_data = function () {
		...
		frappe.call({
			...
			callback: function (r) {
				...
				var sp_el = document.getElementById('crm-f-sp');
				if (sp_el && self.salespersons.length) {
					var current = sp_el.value;
					sp_el.innerHTML = '<option value="">All</option>';
					self.salespersons.forEach(function (sp) {
						var opt = document.createElement('option');
						opt.value = sp.user;
						opt.textContent = sp.full_name || user_label(sp.user);
						sp_el.appendChild(opt);
					});
					if (current) sp_el.value = current;
				}
