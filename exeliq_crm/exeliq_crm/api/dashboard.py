...
def _calculate_kpis(opportunities, leads):
	...
	for opp in opportunities:
		...
		if stage == "Closed Won":    won.append(opp)
		elif stage == "Closed Lost": lost.append(opp)
		else:                        pipeline.append(opp)
...
def _calculate_salesperson_stats(opportunities):
	...
	for opp in opportunities:
		...
		if stage == "Closed Won":
			row["won"] += 1; row["won_value"] += amt
		elif stage == "Closed Lost":
			row["lost"] += 1
		else:
			row["pipeline"] += 1; row["pipeline_value"] += amt
...
def _get_target_data(is_manager, fiscal_years):
	"""Fetch CRM Sales Target records for current quarter and full FY."""
	try:
		fy = fiscal_years.get("current", {})
		if not fy:
			return {"quarterly": [], "annual": [], "meta": {}}
		...
		current_quarter = month_to_quarter.get(today_date.month, "Q1")

		def fetch(extra):
			return frappe.get_all(
				"CRM Sales Target",
				filters={"fiscal_year": fy_label, **extra},
				fields=["user","period_type","quarter","fiscal_year",
				        "revenue_target","leads_target","opportunities_target",
				        "won_deals_target","conversion_target"],
				ignore_permissions=True
			)

		return {
			"quarterly": fetch({"period_type": "Quarterly", "quarter": current_quarter}),
			"annual":    fetch({"period_type": "Annual"}),
			"meta": {
		...
		frappe.log_error(str(e), "CRM Target Fetch Error")
		return {"quarterly": [], "annual": [], "meta": {}}


def _calculate_target_actuals(all_opportunities, all_leads, fiscal_years):
	"""Calculate actuals per user for current quarter and full FY."""
	try:
		fy = fiscal_years.get("current", {})
		if not fy:
			return {}
		...
		from frappe.utils import getdate, today as frappe_today
		today_date = getdate(frappe_today())
		fy_from    = getdate(fy.get("from"))
		fy_to      = getdate(fy.get("to"))
		...
		qm = month_to_q.get(today_date.month, (4,6))
		q_year = today_date.year if qm[0] <= today_date.month else today_date.year - 1
		q_start = date(q_year, qm[0], 1)
		q_end   = date(q_year, qm[1], cal_mod.monthrange(q_year, qm[1])[1])
		...
		def ensure(u):
			if u not in users:
				users[u] = {
					"quarterly": {"revenue":0,"leads":0,"opps":0,"won":0,"lost":0},
					"annual":    {"revenue":0,"leads":0,"opps":0,"won":0,"lost":0}
				}
		...
		for opp in all_opportunities:
			...
			t_date = mk(opp.get("transaction_date"))
			c_date = mk(opp.get("custom_closing_date") or opp.get("transaction_date"))
			amt    = float(opp.get("opportunity_amount") or 0)
			stage  = opp.get("sales_stage", "")

			# Opps created
			if t_date:
				if q_start <= t_date <= q_end:  users[owner]["quarterly"]["opps"] += 1
				if fy_from  <= t_date <= fy_to:  users[owner]["annual"]["opps"]    += 1

			# Won / Lost by closing date
			if c_date:
				if stage == "Closed Won":
					if q_start <= c_date <= q_end: users[owner]["quarterly"]["won"] += 1; users[owner]["quarterly"]["revenue"] += amt
					if fy_from  <= c_date <= fy_to:  users[owner]["annual"]["won"]    += 1; users[owner]["annual"]["revenue"]    += amt
				elif stage == "Closed Lost":
					if q_start <= c_date <= q_end: users[owner]["quarterly"]["lost"] += 1
					if fy_from  <= c_date <= fy_to:  users[owner]["annual"]["lost"]    += 1

		for lead in all_leads:
			owner  = lead.get("lead_owner")
			if not owner: continue
			ensure(owner)
			c_date = mk(lead.get("creation"))
			if not c_date: continue
			if q_start <= c_date <= q_end: users[owner]["quarterly"]["leads"] += 1
			if fy_from  <= c_date <= fy_to:  users[owner]["annual"]["leads"]    += 1

		# Add conversion rates
		for u, periods in users.items():
			for p, s in periods.items():
				closed = s["won"] + s["lost"]
				s["conversion"] = round((s["won"] / closed) * 100, 2) if closed else 0

		return users
	except Exception as e:
		frappe.log_error(str(e), "CRM Target Actuals Error")
		return {}
